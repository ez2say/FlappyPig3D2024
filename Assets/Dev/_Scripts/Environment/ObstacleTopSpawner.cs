...
public class ObstacleTopSpawner : MonoBehaviour
{
    ...
    [SerializeField] private int _maxObstacleCount = 10; // Максимальное количество препятствий
    ...
    IEnumerator SpawnObstacles()
    {
        while (true)
        {
            if (_spawnedObstaclePositions.Count < _maxObstacleCount)
            {
    ...
    private Vector3 GetRandomPositionInArea(BoxCollider area)
    {
        ...
        Vector3 point = new Vector3(
            ...
            5.5f + extents.y, // Генерируем y на верхней границе области спауна
    ...
    private void AdjustObstaclePosition(GameObject obstacle)
    {
        ...
        if (obstacleCollider != null)
        {
            ...
            // Корректировка позиции, чтобы препятствие было на потолке
            obstaclePosition.y = 80f - obstacleHeight / 2f;
            obstacle.transform.position = obstaclePosition;

            // Поворачиваем препятствие вниз
            obstacle.transform.rotation = Quaternion.Euler(180f, 0f, 0f);
        }
    }
}